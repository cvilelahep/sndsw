...
namespace snd::analysis_cuts{
  ...
  void trackSciFiDOCA::process(){
	...
	float avg_ver = std::accumulate(min_doca_ver.begin(), min_doca_ver.end(), 0,
					[](float sum, float element){return element < 9.9e9 ? sum+element: sum;})/n_ver;
	float avg_hor = std::accumulate(min_doca_hor.begin(), min_doca_hor.end(), 0,
					[](float sum, float element){return element < 9.9e9 ? sum+element: sum;})/n_hor;
